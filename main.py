...
async def sb_count_users() -> int:
    ...
    if r.status_code == 200:
        # count is in Content-Range: 0-0/123
...
async def sb_upsert_user(tg_uid: int, email: str, is_founder: bool):
    ...
    # 409 often means unique violation on email
...
async def capture_email(payload: EmailPayload, request: Request):
    ...
    # If you require binding, enforce it
    if tg_uid is None:
    ...
    # Optional strong verification (recommended)
    ...
    # Determine founder flag
    is_founder = bool(existing_by_tg.get("is_founder")) if existing_by_tg else False
    if not is_founder:
        # If we still have founder slots, assign on first-time user
        # (Best-effort; DB trigger version is provided in SQL for strictness.)
        count = await sb_count_users()
        # If user doesn't exist yet, they will become count+1
    ...
    # If a later user re-binds and count moved, never demote founder
